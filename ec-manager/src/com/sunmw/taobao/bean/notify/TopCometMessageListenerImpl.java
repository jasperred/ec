...
public class TopCometMessageListenerImpl implements TopCometMessageListener {
	...
	public void onReceiveMsg(String message) {
		try {
			NotifyMessageThread nmt = new NotifyMessageThread();
			nmt.setStoreId(storeId);
			nmt.setMessage(message);
			nmt.start();
		} catch (Exception e) {
	...
	{
		try {
			NotifyDiscardThread ndt = new NotifyDiscardThread();
			ndt.setStoreId(storeId);
			ndt.setMessage(message);
			ndt.start();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
