...
public class TBOrderServicesByFastImpl extends HibernateDaoSupport implements
		TBOrderServices {
	static Logger logger = Logger.getLogger(TBOrderServicesByFastImpl.class);
	...
	public Map taobaoTradesSoldIncrementGet(Map params) {
		...
		while (true) {
			try {
				TradesSoldIncrementGetResponse tr = client.execute(ts, tbStore
						.getSessionKey());
				// 接口执行成功
				if (tr.isSuccess()) {
					...
					for (Trade trade : trades) {
						taobaoTradeFullinfo(trade.getTid(),
	...
	public Map taobaoTradesSoldIncrementVGet(Map params) {
		...
		while (true) {
			try {
				TradesSoldIncrementvGetResponse tr = client.execute(ts, tbStore
						.getSessionKey());
				// 接口执行成功
				if (tr.isSuccess()) {
					//logger.info("淘宝API执行成功,page[" + pageNo + "]");
	...
	public Map taobaoTradesSoldGet(Map params) {
		...
		while (true) {
			try {
				TradesSoldGetResponse tr = client.execute(ts, tbStore
						.getSessionKey());
				// 接口执行成功
				if (tr.isSuccess()) {
					//logger.info("淘宝API执行成功,page[" + pageNo + "]");
	...
	// 获得订单的全部信息,并保存到数据库
	//此方法使用同步，以保证数据的唯一性
	public synchronized void taobaoTradeFullinfo(long tid, String fields,
		...
		try {
			...
			if (tf.isSuccess()) {
				...
				if (tbTrade.getId() == null)
					session.save(tbTrade);
				else
					session.update(tbTrade);
				...
				if (orders == null || orders.size() == 0) {
					...
					for (Order o : orders) {
						...
						List<TbOrder> tbOrderList = session.createQuery(
				...
				if (promotionDetailList != null
					...
					for (PromotionDetail pd : promotionDetailList) {
						List<TbPromotionDetail> tbpdlist = session.createQuery(
		...
		logger.info("tb time:"+(System.currentTimeMillis()-c));
	}
