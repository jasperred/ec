...
public class CopyOfOrderServicesImpl extends HibernateDaoSupport implements
		OrderServices {

	static Logger logger = Logger.getLogger(CopyOfOrderServicesImpl.class);
	...
	public Map getOrderInfo(int orderHeadId) {
		...
		if (silist != null) {
			for (StatusItem si : silist) {
				...
				else //(si.getStatusTypeId().equals("TAOBAO_ORDER"))
	...
	public synchronized String updateOrderFromTb(List<Map> tbOrderList) {
		...
		for (Map m : tbOrderList) {
			...
			if (WebUtil.isNull(oh.getOrderNo()))
				oh.setOrderNo(genericOrderNo("TBOD", 6, "yyyyMMdd", session));
	...
	public Map updateSingleOrderFromTb(Map trade, Integer storeId) {
		...
		try {
			...
			if (toList != null) {
				for (Map to : toList) {
					if (to.get("Price") != null && to.get("Num") != null)
						orderAmt = orderAmt.add(new BigDecimal(to.get("Price").toString())
								.multiply(new BigDecimal(to.get("Num").toString())));
			...
			if (WebUtil.isNull(oh.getStoreId())) {
				...
				List<Integer> sl = session.createQuery("select CompanyId from Store where id = "+storeId).list();
			...
			if (toList != null) {
				...
				for(Map to : toList)
				{
					if (WebUtil.isNotNull(to.get("TotalFee")))
						detailAmt = detailAmt.add((BigDecimal)(to.get("TotalFee")));
				}
				//已计算的整单折扣金额
				...
				for (Map to : toList) {
					...
					if (oi == null) {
						...
						if (WebUtil.isNotNull(to.get("DiscountFee")))
							oi.setDiscountPriceAmt((BigDecimal)(to.get("DiscountFee")));
						
						oi.setSkuCd((String)to.get("OuterSkuId"));
						// 销售属性处理[拖鞋尺码:其它;颜色分类:天蓝色;规格:中号]
						if (WebUtil.isNotNull(to.get("SkuPropertiesName"))) {
							String[] props = to.get("SkuPropertiesName").toString().split(";");
							for (String prop : props) {
								String[] ps = prop.split(":");
								if (ps.length == 2) {
									if (ps[0].indexOf("颜色") >= 0) {
						...
						//折扣价格=销售价格-明细金额/明细数量
						oi.setDiscountPrice(oi.getPrice().subtract(oi.getSubTotal().divide(oi.getReqQty(),2,BigDecimal.ROUND_HALF_UP)));
						//折扣小计=销售价格*明细数量-明细金额
						oi.setDiscountPriceAmt(oi.getPrice().multiply(oi.getReqQty()).subtract(oi.getSubTotal()));
						//扣除OrderHead折扣计算
						if(r==toList.size())
						{
							...
							//商品实际单价=商品实际总额/明细数量
							oi.setRealPrice(oi.getRealPriceAmt().divide(oi.getReqQty(),2,BigDecimal.ROUND_HALF_UP));
						}
						else
						{
							//商品实际总额=支付货款*（明细金额/明细总额）
							BigDecimal c = amt.multiply(oi.getSubTotal().divide(detailAmt,2,BigDecimal.ROUND_HALF_UP));							
							oi.setRealPriceAmt(c);
							//商品实际单价=商品实际总额/明细数量
							oi.setRealPrice(c.divide(oi.getReqQty(),2,BigDecimal.ROUND_HALF_UP));
						}
						session.save(oi);
					} else {
						oi.setMtime(date);
	...
	public Map updateSingleOrderFromPp(Map order, Integer storeId) {
		...
		try {
			...
			if (order.get("dealState").equals("DS_WAIT_BUYER_RECEIVE"))
				oh.setOrderStatus("DELIVERY");
			// 交易成功,订单完成
			if (order.get("dealState").equals("DS_DEAL_END_NORMAL")||order.get("dealState").equals("DS_BUYER_EVALUATED"))
				oh.setOrderStatus("COMPLETE");
			// 订单未处理时才更新订单状态(除'关闭'状态订单)
			if (WebUtil.isNull(oh.getOrderStatus())
					|| oh.getOrderStatus().equals("START")) {
				...
				if (order.get("dealState").equals("DS_WAIT_BUYER_PAY")
						||
						// 已付款状态但保留原因不是未支付，订单状态设置为‘未审核’
						(order.get("dealState").equals("DS_WAIT_SELLER_DELIVERY")
								&& WebUtil.isNotNull(oh.getReceiveField1()) && !oh
								.getReceiveField1().equals("I"))) {
					oh.setOrderStatus("START");
				}
			...
			if (toList != null) {
				for (Map to : toList) {
					if (WebUtil.isNotNull(to.get("itemDealPrice")) && WebUtil.isNotNull(to.get("itemDealCount")))
					{
						orderAmt = orderAmt.add(new BigDecimal(to.get("itemDealPrice").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP)
								.multiply(new BigDecimal(to.get("itemDealCount").toString())));
						orderItemTotalFee = orderItemTotalFee.add(new BigDecimal(to.get("itemDealPrice").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP)
						.multiply(new BigDecimal(to.get("itemDealCount").toString())));
					}
					if(WebUtil.isNotNull(to.get("itemDiscountFee")))
						orderItemTotalFee = orderItemTotalFee.subtract(new BigDecimal(to.get("itemDiscountFee").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
					if(WebUtil.isNotNull(to.get("itemAdjustPrice")))
							orderItemTotalFee = orderItemTotalFee.add(new BigDecimal(to.get("itemAdjustPrice").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
			...
			if (WebUtil.isNull(oh.getOrderNo()))
				oh.setOrderNo(genericOrderNo("PPOD", 6, "yyyyMMdd", session));
			if (WebUtil.isNull(oh.getOrderType()))
				oh.setOrderType("ORDER");
			if (WebUtil.isNull(oh.getId()))
				oh.setCtime(date);
			...
			// 发货时间
			if (WebUtil.isNotNull(order.get("sellerConsignmentTime"))&&WebUtil.toDateForString(order.get("sellerConsignmentTime").toString(), "yyyy-MM-dd HH:mm:ss").after(WebUtil.toDateForString("1970-01-01 08:00:00", "yyyy-MM-dd HH:mm:ss"))
					&& WebUtil.isNull(oh.getDeliveryDate())) {
				oh.setDeliveryDate(WebUtil.toDateForString(order.get("sellerConsignmentTime").toString(), "yyyy-MM-dd HH:mm:ss"));
			}
			// 支付时间
			if (WebUtil.isNotNull(order.get("payTime"))&&WebUtil.toDateForString(order.get("payTime").toString(), "yyyy-MM-dd HH:mm:ss").after(WebUtil.toDateForString("1970-01-01 08:00:00", "yyyy-MM-dd HH:mm:ss")))
			oh.setPayTime(WebUtil.toDateForString(order.get("payTime").toString(), "yyyy-MM-dd HH:mm:ss"));
			// 支付帐号
			...
			if (WebUtil.isNull(oh.getStoreId())) {
				oh.setStoreId(storeId);
				List<Object[]> sl = session.createQuery("select CompanyId,StoreName from Store where id = "+storeId).list();
				if(!WebUtil.isNullForList(sl))
				{
			...
			if (WebUtil.isNotNull(order.get("couponFee")))
				payment = payment.add(new BigDecimal(order.get("couponFee").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
			if (WebUtil.isNotNull(order.get("freight")))
				payment = payment.add(new BigDecimal(order.get("freight").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
			oh.setPayment(payment);
			// 邮费
			if (WebUtil.isNotNull(order.get("freight")))
				oh.setPostFee(new BigDecimal(order.get("freight").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
			else
				oh.setPostFee(new BigDecimal(0));
			// 折扣
			if (WebUtil.isNotNull(order.get("couponFee")))
				oh.setDiscount(new BigDecimal(order.get("couponFee").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(-1)));
			else
				oh.setDiscount(new BigDecimal(0));
			// 积分
			if (WebUtil.isNotNull(order.get("dealPayFeePoint")))
				oh.setPaymentPoint(new Integer(order.get("dealPayFeePoint").toString()));
			else
				oh.setPaymentPoint(0);
			// 订单时间取支付时间
			if (WebUtil.isNotNull(order.get("payTime"))&&WebUtil.toDateForString(order.get("payTime").toString(), "yyyy-MM-dd HH:mm:ss").after(WebUtil.toDateForString("1970-01-01 08:00:00", "yyyy-MM-dd HH:mm:ss")))
				oh.setOrderDate(WebUtil.toDateForString(order.get("payTime").toString(), "yyyy-MM-dd HH:mm:ss"));
			if (WebUtil.isNull(oh.getId())) {
				if (WebUtil.isNotNull(order.get("createTime"))&&WebUtil.toDateForString(order.get("createTime").toString(), "yyyy-MM-dd HH:mm:ss").after(WebUtil.toDateForString("1970-01-01 08:00:00", "yyyy-MM-dd HH:mm:ss")))
				oh.setRequestDate(WebUtil.toDateForString(order.get("createTime").toString(), "yyyy-MM-dd HH:mm:ss"));
			}
			if (isNew)
				session.save(oh);
			else
				...
				for (Map to : toList) {
					...
					if (oi == null) {
						...
						if (WebUtil.isNotNull(to.get("itemDealCount")))
							oi.setQty(new BigDecimal(to.get("itemDealCount").toString()));
						if (WebUtil.isNotNull(to.get("itemDealPrice")))
							oi.setPrice(new BigDecimal(to.get("itemDealPrice").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
						if (WebUtil.isNotNull(to.get("itemDealCount")))
							oi.setReqQty(new BigDecimal(to.get("itemDealCount").toString()));
						if (WebUtil.isNotNull(to.get("itemDiscountFee")))
							oi.setDiscountPriceAmt(new BigDecimal(to.get("itemDiscountFee").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
						if (WebUtil.isNotNull(to.get("itemAdjustPrice")))
							oi.setAdjustFee(new BigDecimal(to.get("itemAdjustPrice").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
						
						oi.setSkuCd((String)to.get("stockLocalCode"));
						// 销售属性处理[拖鞋尺码:其它;颜色分类:天蓝色;规格:中号]
						if (WebUtil.isNotNull(to.get("stockAttr"))) {
							String[] props = to.get("stockAttr").toString().split("\\|");
							for (String prop : props) {
								String[] ps = prop.split(":");
								if (ps.length == 2) {
									if (ps[0].indexOf("颜色") >= 0) {
						...
						if (WebUtil.isNotNull(to.get("itemDealPrice")) && WebUtil.isNotNull(to.get("itemDealCount")))
						{
							subTotal = subTotal.add(new BigDecimal(to.get("itemDealPrice").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP)
							.multiply(new BigDecimal(to.get("itemDealCount").toString())));
						}
						if(WebUtil.isNotNull(to.get("itemDiscountFee")))
							subTotal = subTotal.subtract(new BigDecimal(to.get("itemDiscountFee").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
						if(WebUtil.isNotNull(to.get("itemAdjustPrice")))
							subTotal = subTotal.add(new BigDecimal(to.get("itemAdjustPrice").toString()).divide(new BigDecimal(100),2,BigDecimal.ROUND_HALF_UP));
						oi.setSubTotal(subTotal);
						//折扣价格=销售价格-明细金额/明细数量
						if(oi.getDiscountPriceAmt()!=null)
						oi.setDiscountPrice(oi.getDiscountPriceAmt().divide(oi.getReqQty(),2,BigDecimal.ROUND_HALF_UP));
						//扣除OrderHead折扣计算
						if(r==toList.size())
						{
							//商品实际总额=支付货款-已计算的整单折扣金额
						...
						{
							//商品实际总额=支付货款*（明细金额/明细总额）
							BigDecimal c = amt.multiply(oi.getSubTotal().divide(orderItemTotalFee,2,BigDecimal.ROUND_HALF_UP));							
							oi.setRealPriceAmt(c);
							//商品实际单价=商品实际总额/明细数量
							oi.setRealPrice(c.divide(oi.getReqQty(),2,BigDecimal.ROUND_HALF_UP));
						}
