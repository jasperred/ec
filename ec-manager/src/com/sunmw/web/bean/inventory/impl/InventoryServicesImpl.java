...
public class InventoryServicesImpl extends HibernateDaoSupport implements
	...
	public void clearInventory(int storeId)
	{
		...
		session.createQuery("delete from Inventory where WhId = '"+storeId+"'").executeUpdate();
		session.createQuery("delete from InventoryLog where WhId = '"+storeId+"'").executeUpdate();
		tx.commit();
		session.close();
	}
	...
	public Map inventoryProcess(Map param)
	{
		...
		Transaction tx = session.beginTransaction();
		List<Warehouse> wl = session.createQuery("from Warehouse where Status = 'AVAILABLE'").list();
		Map<String,Integer> whMap = new HashMap();
		if(!WebUtil.isNullForList(wl))
		{
			for(Warehouse w:wl)
			{
				whMap.put(w.getWhCode(), w.getId());
			}
		}
		Date date = new Date();
		for(Map m:invList)
		{
			...
			Integer invId = null;
			//查询库存,必须是A品
			List<Inventory> localInvList = session.createQuery("from Inventory where InvStatus = 'ACTIVE' and WhId = '"+whId+"' and SkuCd = '"+skuCd+"' and InvType = 'A'").list();
			//无库存
			...
			else
			{
				...
				if(ioType.equals("I"))
				{
					inv.setAvailableQuantity(new BigDecimal(inv.getAvailableQuantity().intValue()+qty));
					inv.setQuantity(new BigDecimal(inv.getQuantity().intValue()+qty));
				}
				//替换
	...
	{
		List<Warehouse> whList = this.getHibernateTemplate().find("from Warehouse where Status = 'ACTIVE' and StoreId = "+storeId);
	...
	public List<Map> getTbUpdateInventory(int storeId)
	{
		...
		List<Inventory> invList = this.getHibernateTemplate().find("from Inventory where InvStatus = 'ACTIVE' and WhId = '"+whId+"' and InvType = 'A'");
		if(WebUtil.isNullForList(invList))
		{
	...
	public void tbOrderInventoryProcess(int storeId)
	{
		...
		String origOrderStatus = WebConfigProperties.getProperties("inventory.taobao.status");
		Session session = this.getSession();
		List<OrderHead> ohl = session.createQuery("from OrderHead where OrderType = 'ORDER' and (InvStatus != 'INV' or InvStatus is null) and OrigOrderStatus in ("+origOrderStatus+") and WhId = '"+storeId+"'").list();
		...
		for(OrderHead oh:ohl)
		{
			...
			List<OrderItem> oil = session.createQuery("from OrderItem where OrderHeadId = "+oh.getId()).list();
			if(WebUtil.isNullForList(oil))
			{
				logger.error("订单["+oh.getOrderNo()+"],淘宝订单号["+oh.getOrigOrderNo()+"]没有明细内容");
				continue;
			}
	...
	{
		StringBuffer hql = new StringBuffer(" from Inventory inv,SkuMaster sm,ItemMaster im where inv.SkuCd = sm.SkuCd and sm.ItemId = im.id");
	...
	public Map updateInvToUnitSku(Map param)
	{
		...
		if(hasNoPay==null)
			hasNoPay = "N";
		//未支付状态
		String origOrderStatusOfNoPay = (String) param.get("OrigOrderStatusOfNoPay");
		if(origOrderStatusOfNoPay==null)
			origOrderStatusOfNoPay = "'DS_WAIT_BUYER_PAY','TRADE_NO_CREATE_PAY','WAIT_BUYER_PAY'";
		List<Object[]> whList = this.getHibernateTemplate().find("from Store s,Warehouse w where w.Status = 'ACTIVE' and s.WhId = w.id and s.id = "+storeId);
		if(WebUtil.isNullForList(whList))
		{
			result.put("Flag", "error");
		...
		List<Inventory> invList = this.getHibernateTemplate().find("from Inventory where UpdateFlag = 'N' and WhId = ? and UpdateTime >= ?",new Object[]{w.getId(),WebUtil.formatDateString(date, "yyyy-MM-dd HH:mm:ss")});
		if(WebUtil.isNullForList(invList))
		{
			result.put("Flag", "error");
		...
		List<Store> storeList = this.getHibernateTemplate().find("from Store where WhId = ? order by Level",w.getId());
		...
		List<InvRule> invRuleList = this.getHibernateTemplate().find("from InvRule where WhId = ?",w.getId());
		//商品安全库存查询
		List<Object[]> skuReserveInvList = this.getHibernateTemplate().find("select s.SkuCd,s.ReserveInv skuInv,i.ReserveInv itemInv from ItemMaster i,SkuMaster s where i.id = s.ItemId");
		...
		if(!WebUtil.isNullForList(skuReserveInvList))
		{
			for(Object[] skus:skuReserveInvList)
			{
				...
				skuReserveInvMap.put((String)skus[0], WebUtil.isNull(skus[1])||skus[1].equals(new Integer(0))?(Integer)skus[2]:(Integer)skus[1]);
			}
		}
		//未发给仓库订单查询
		...
		{
			orderSql = "select i.SkuCd,i.ReqQty from OrderHead h,OrderItem i where h.id = i.OrderHeadId and h.OrderType = 'ORDER' and (h.OrderStatus = 'START' or h.OrderStatus = 'AUDITED') and h.StoreId in ("+storeIdStr.toString()+")";
		}
		else
		{
			orderSql = "select i.SkuCd,i.ReqQty from OrderHead h,OrderItem i where h.id = i.OrderHeadId and h.OrderType = 'ORDER' and (h.OrderStatus = 'START' or h.OrderStatus = 'AUDITED') and OrigOrderStatus not in ("+origOrderStatusOfNoPay+") and h.StoreId in ("+storeIdStr.toString()+")";
		}
		...
		if(!WebUtil.isNullForList(orderList))
		{
			...
			{
				orderInvMap.put((String)oi[0], WebUtil.isNull(oi[1])?0:((BigDecimal)oi[1]).intValue());
			}
		}
		//无库存规则的商品，每个店使用相同的库存
		Session session = this.getSession();
		try {
			if(WebUtil.isNullForList(invRuleList)||WebUtil.isNull(invRuleList.get(0).getProportionStr()))
			{
				for(Inventory inv:invList)
				{
					Transaction tx = session.beginTransaction();
					...
					{
						session.createQuery("update UnitSkuMaster set InvUpdateTime='"+WebUtil.formatDateString(inv.getUpdateTime(), "yyyy-MM-dd HH:mm:ss")+"',InvUpdateFlag='Y',SkuInv="+(inv.getAvailableQuantity().subtract(new BigDecimal(reserveInv)))+" where SkuCd = '"+inv.getSkuCd()+"' and StoreId = "+store.getId()).executeUpdate();
					}
					//更新库存标记
					session.createQuery("update Inventory set UpdateFlag = 'Y',Mtime = '"+WebUtil.formatDateString(new Date(), "yyyy-MM-dd HH:mm:ss")+"' where SkuCd = '"+inv.getSkuCd()+"' and WhId = "+w.getId()).executeUpdate();
					tx.commit();
				}
			}
			else
			{
				...
				for(String pp:ppStr)
				{
					...
					{
						result.put("Flag", "error");
						result.put("Message", "仓库【"+w.getWhName()+"-"+w.getWhCode()+"】的比例参数不正确");
						session.close();
						return result;
					}
					...
					{
						result.put("Flag", "error");
						result.put("Message", "仓库【"+w.getWhName()+"-"+w.getWhCode()+"】的比例参数不正确,店铺不存在");
						session.close();
						return result;
					}
					...
					if(new Integer(pps[1])>0)
					{
						...
						stPpList.add(WebUtil.toMap("storeId", new Integer(pps[0]),"scale",new Integer(pps[1])));
					}
				}
				//按店铺比例排序
				...
				for(Inventory inv:invList)
				{
					...
					if(qty<=stPpMap.size())
					{
						for(Store st:storeList)
						{
							...
							{
								session.createQuery("update UnitSkuMaster set InvUpdateTime='"+WebUtil.formatDateString(inv.getUpdateTime(), "yyyy-MM-dd HH:mm:ss")+"',InvUpdateFlag='Y',SkuInv = 0 where SkuCd = '"+inv.getSkuCd()+"' and StoreId = "+st.getId()).executeUpdate();
							}
						}
						...
						for(int i=stPpList.size()-1;i>=0;i--)
						{
							...
							if(qty<=0)
							{
								session.createQuery("update UnitSkuMaster set InvUpdateTime='"+WebUtil.formatDateString(inv.getUpdateTime(), "yyyy-MM-dd HH:mm:ss")+"',InvUpdateFlag='Y',SkuInv = 0 where SkuCd = '"+inv.getSkuCd()+"' and StoreId = "+stId).executeUpdate();
							}
							//库存更新为1
							else
							{
								session.createQuery("update UnitSkuMaster set InvUpdateTime='"+WebUtil.formatDateString(inv.getUpdateTime(), "yyyy-MM-dd HH:mm:ss")+"',InvUpdateFlag='Y',SkuInv = 1 where SkuCd = '"+inv.getSkuCd()+"' and StoreId = "+stId).executeUpdate();
								qty--;
							}
						}
					}
					...
					else
					{
						...
						for(Map m:stPpList)
						{
							...
							BigDecimal scale = new BigDecimal(p).divide(new BigDecimal(total),2,BigDecimal.ROUND_HALF_UP);//库存比例计算
							BigDecimal skuInv = new BigDecimal(qty).multiply(scale);//可分配库存
							int skuinv = 0;
							...
							session.createQuery("update UnitSkuMaster set InvUpdateTime='"+WebUtil.formatDateString(inv.getUpdateTime(), "yyyy-MM-dd HH:mm:ss")+"',InvUpdateFlag='Y',SkuInv = "+skuinv+" where SkuCd = '"+inv.getSkuCd()+"' and StoreId = "+stId).executeUpdate();
	...
	public Map getAvailableInvOfShop(Map param) {
		...
		//查询仓库及库存有效时间
		List<Object[]> wsList = this.getHibernateTemplate().find("select w.id,w.ValidTime from Store s,Warehouse w where w.Status = 'ACTIVE' and s.WhId = w.id and s.id = "+storeId);
		...
		List<Object[]> skuList = this.getHibernateTemplate().find("select SkuCd,SkuInv from UnitSkuMaster where InvUpdateFlag = 'N' and InvUpdateTime >= '"+WebUtil.formatDateString(date, "yyyy-MM-dd HH:mm:ss")+"'");
		...
		{
			l.add(WebUtil.toMap("SkuCd", o[0],"Qty",((BigDecimal)o[1]).intValue()));
		}
		result.put("Flag", "success");
	...
	public Map updateInvFlagOfShop(Map param)
	{
		...
		this.getHibernateTemplate().bulkUpdate("update UnitSkuMaster set InvUpdateFlag = 'Y' where StoreId = "+storeId);
		result.put("Flag", "success");
		return result;
	}
}
