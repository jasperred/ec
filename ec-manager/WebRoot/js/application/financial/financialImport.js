...
Ext.onReady(function() {
	...
	var store = Ext.create('Ext.data.Store', {
		...
		proxy : new Ext.data.HttpProxy({
					...
					type : 'jsonp',
					url : 'fileListAction?filePath=financial.import.path&fileUrl=',
					reader : {
						root : 'fileList',
						fields : ['fileName', 'fileDate', 'filePath', 'fileUrl']
					},
	...
	resultGrid.child('pagingtoolbar').add(['->', {
		...
		handler : function() {
			...
			Ext.Ajax.request({
						...
						params : {
							filePath : 'financial.import.path',
		...
		handler : function() {
			if (!storeWin) {
				storeWin = Ext.widget('window', {
					title : '请选择本次导入的报表所在的店铺',
					closeAction : 'hide',
					width : 300,
					height : 60,
					layout : 'fit',
					...
					buttons : [{
						...
						handler : function() {
							...
							if (storeId.length == 0) {
								Ext.MessageBox.alert('提示', "请选择店铺!");
								return;
							}
							storeWin.hide();
							Ext.Msg.wait('处理中，请稍后...', '提示');
							Ext.Ajax.request({
								url : 'importAlipayFinancialReportCsv',
								timeout : 3600000,
								params : {
									storeId : storeId,
						...
						text : "取消",
						handler : function() {
							Ext.getCmp('s-store-id').setValue('');
							storeWin.hide();
						}
					}]
				});
