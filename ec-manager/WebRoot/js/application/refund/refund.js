...
Ext.onReady(function() {
	...
	resultGrid.addListener('itemdblclick', showRefundHandler, this);
		...
		store.on('beforeload', function() { // =======翻页时 查询条件
					var new_params = {
						...
						refundEndDate : Ext.getCmp("s-order-end-date")
								.getValue(),
						completeFromDate : Ext.getCmp("s-complete-from-date")
								.getValue(),
						completeEndDate : Ext.getCmp("s-complete-end-date")
								.getValue()
					};
	...
	function exportHandler() {
		Ext.Ajax.request({
			...
			params : {
				limit : 100000,
				storeId : Ext.getCmp("s-store-id").getValue(),
				refundPropsValue : Ext.getCmp("s-order-props-value").getValue(),
				refundPropsName : Ext.getCmp("s-order-props-name").getValue(),
				tbStatus : Ext.getCmp("s-tb-status").getValue(),
				erpStatus : Ext.getCmp("s-erp-status").getValue(),
				refundStatus : Ext.getCmp("s-refund-status").getValue(),
				goodStatus : Ext.getCmp("s-good-status").getValue(),
				hasGoodReturn : Ext.getCmp("s-has-good-return").getValue(),
				refundFromDate : Ext.getCmp("s-order-from-date").getValue(),
				refundEndDate : Ext.getCmp("s-order-end-date").getValue(),
				completeFromDate : Ext.getCmp("s-complete-from-date")
						.getValue(),
				completeEndDate : Ext.getCmp("s-complete-end-date").getValue()
			},
			success : function(response, options) {
				...
				if (Ext.String.trim(responseArray.exportUrl).length > 0) {
					window.open(Ext.String.trim(responseArray.exportUrl));
				} else {
		...
		Ext.Ajax.request({
			...
			success : function(response, options) {
				...
				if (responseArray.OrderHead.id) {
					...
					if (responseArray.RefOrderHead != null)
						Ext.getCmp("buyer-name")
								.setValue(responseArray.RefOrderHead.buyerNick);
					...
					if (responseArray.RefOrderHead != null)
						Ext.getCmp("alipay-no")
								.setValue(responseArray.RefOrderHead.payNo);
					Ext.getCmp("payment")
					...
					Ext.getCmp("refund-date").setValue(Ext.Date.format(
							new Date(responseArray.OrderHead.requestDate),
							'Y-m-d H:i:s'));
						...
						Ext.Ajax.request({
							...
							success : function(response, options) {
								...
								if (responseArray != null
									...
									Ext
											.getCmp("alipay-no")
											.setValue(responseArray.OrderHead.alipayNo);
									Ext
