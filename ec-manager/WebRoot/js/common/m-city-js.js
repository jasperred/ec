function initCity(stateId, cityId, districtId) {
	...
	var stateIndex;
	//省份选择
	state.on('change', function(combo, newValue, oldValue, eOpts) {
				//清除城市和地区数据，重新加载
				cityStore.removeAll();
				...
				city.setValue("");
				district.setValue("");
				stateIndex = stateStore.find("name",newValue);
				if (stateIndex <= 0)
					return;
				cityStore.add(cityArray[stateIndex - 1]);
			}, this);
	//城市选择
	city.on('change', function(combo, newValue, oldValue, eOpts) {
				//清除地区数据，重新加载
				districtStore.removeAll();
				districtStore.add({
							"name" : "请选择地区",
				...
				var cityIndex = cityStore.find("name",newValue);
				district.setValue("");
